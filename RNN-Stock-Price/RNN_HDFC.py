...
#training_set = pd.read_csv('HDFCBANK_train.csv')

#training_set = pd.read_csv('Google_Stock_Price_Train.csv')
training_set = pd.read_csv('h2012.csv').convert_objects(convert_numeric=True)
training_set.describe()
training_set = training_set.fillna(method='ffill') #if you have missing data
training_set.describe()
...
X_train = training_set[0:1211] # first to last but one
y_train=training_set[1:1212]
...
X_train = np.reshape(X_train,(1211,1,1))
...
test_set = pd.read_csv('HDFCBANK_test.csv').convert_objects(convert_numeric=True)
test_set = test_set.fillna(method='ffill') #if you have missing data
